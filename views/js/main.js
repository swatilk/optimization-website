...
/**
 *  @description This function Pulls adjective out of array using random number sent from generator
 *  @param{string} x - string for adjective
*/
function getAdj(x){
  switch(x) {
    case "dark":
...
/** @description This function Pulls noun out of array using random number sent from generator
*/
function getNoun(y) {
  switch(y) {
    case "animals":
...
/**
 *  @description This fucntion generates random numbers for getAdj and getNoun functions and returns a new pizza name
 */
function generator(adj, noun) {
 ...
 /**
  * @description This function Chooses random adjective and random noun
  */
function randomName() {
 ...
 *  @param{number} i - index of the pizza array
 */
...
/**
 *  @description resizePizzas(size) is called when the slider in the "Our Pizzas" section of the website moves.
 *  @param {number} size - size of the pizza (1, 2 or 3)
 */
var resizePizzas = function(size) {
  window.performance.mark("mark_start_resize");   // User Timing API function

  /**
   *  @description This function changes the value for the size of the pizza above the slider
   *  @param {number} size - size of the pizza (1, 2, or 3)
   */
  ...
  /**
   *  @description This function returns the size difference to change a pizza element from one size to another. Called by changePizzaSlices(size).
   *  @param {HTML Element} elem - The container of the pizza image
   *  @param {number} size - The size of the pizza image (1, 2 or 3)
   */
 ...
 *  @param {number} times - The array of User Timing measurements from updatePositions()
 */
...
function updatePositions() {
  ...
  phaseArray.push(Math.sin( scroller + 0) * 700);
  phaseArray.push(Math.sin( scroller + 1) * 700);
  phaseArray.push(Math.sin( scroller + 2) * 700);
  phaseArray.push(Math.sin( scroller + 2) * 700);
  phaseArray.push(Math.sin( scroller + 3) * 700);
  phaseArray.push(Math.sin( scroller + 4) * 700);
  for (var i = 0; i < len; i++) {
    items[i].style.transform = 'translateX(' + (phaseArray[items[i].phase]) +'px)';
  }
  // User Timing API
